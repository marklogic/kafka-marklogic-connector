...
class DslConstraintInjectionPollTest extends AbstractIntegrationSourceTest {
    ...
    void orderAuthorsByDateAndLimit3() throws InterruptedException {
        ...
        assertEquals("2022-04-11", task.getPreviousMaxConstraintColumnValue(), "Did not get the expected 3rd lowest date");

        records = task.poll();
        assertEquals(3, records.size());
        assertEquals("2022-05-11", task.getPreviousMaxConstraintColumnValue(), "Did not get the expected 6th lowest date");
    }
